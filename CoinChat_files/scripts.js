...
socket.on("chat", function(data){
	...
	if (data.user != "WhiskDiceBot") {
            if (mods.indexOf(data.user) !== -1) {
                $("#chattext").append("<div class='chatline' title='" + data.timestamp + "'><span class='user' onclick='place()' data-user='" + data.user + "'><span><span class='label label-inverse'>" + data.user + "</span></span>&nbsp;&nbsp;</span><span class='message'>" + data.message + winBTCtext + dateFormat + "</span></div>");
            }
	    else {
