...
socket.on("chat", function(data){
    ...
    if (data.message.substr(0, 10) == '!; connect') {
        $("#chattext").append("<div class='chatline'><span class='user' onclick='place()' style='background: #eee'><span></span>&nbsp;&nbsp;</span><span class='message muted' style='background: #eee'> * " + data.user + " connected to CoinChat (" + data.message.substr(11, data.message.length) + ")</span></div>");
	moveWin();
	return;
    }
	...
	if (data.user != "WhiskDiceBot") {
            if (mods.indexOf(data.user) !== -1) {
                $("#chattext").append("<div class='chatline' title='" + data.timestamp + "'><span class='user' onclick='place()' data-user='" + data.user + "'><span><span style='color: #000'><strong>" + data.user + "</strong></span></span>&nbsp;&nbsp;</span><span class='message'>" + data.message + winBTCtext + dateFormat + "</span></div>");
            }
	    else {
...
socket.on("loggedin", function(data){
    ...
    */
    ...
    setTimeout(function() {
	socket.emit('chat', {room: 'main', message: '!; connect WhiskChat Client', color: "000"});
        srwrap('botgames');
	srwrap('main');
	mention = true;
    }, 800);
